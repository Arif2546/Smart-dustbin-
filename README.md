...
<head>
  ...
  <title>Smart Reward Based Dustbin | Arif Mehboob</title>
  <meta name="description" content="Ultrasonic, Servo, LCD ও Buzzer দিয়ে তৈরি Smart Reward Based Dustbin – প্রজেক্টের কাজ, ফিচার, গ্যালারি ও কোড।" />
  
  <meta name="author" content="Arif Mehboob">
  <meta property="og:title" content="Smart Reward Based Dustbin by Arif Mehboob">
  <meta property="og:description" content="A project that promotes a clean environment using automated and reward-based technology.">
  <meta property="og:image" content="https://i.ibb.co/27yFnRGf/IMG-20250820-WA0033.jpg">
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .nav{
      ...
      background:rgba(216, 17, 89, .8); border-bottom:1px solid rgba(0,0,0,.15);
    ...
    .brand{font-weight:800; letter-spacing:.3px; font-size:1.1rem; color:var(--white);}
    .menu a{margin:0 10px; padding:8px 10px; border-radius:10px; transition: background .2s ease, color .2s ease;}
    .menu a:hover{background:var(--brand-2); color:var(--white);}
    .menu a.active{background:var(--white); color:var(--brand); font-weight:700;}
    
    /* Hero */
    .hero{max-width:1100px;margin:auto;padding:48px 16px 32px;display:grid;grid-template-columns:1.2fr .8fr;gap:24px}
    .badge{display:inline-block;background:rgba(216, 17, 89, .15);border:1px solid rgba(216, 17, 89, .35);color:var(--brand);padding:6px 10px;border-radius:999px;font-size:12px; font-weight:600;}
    h1{font-size:clamp(28px,4vw,44px);line-height:1.15;margin:12px 0 8px; font-weight:800; color:var(--text);}
    ...
    .btn{background:var(--brand);border:0;color:var(--white);padding:12px 20px;border-radius:12px;font-weight:700; transition: transform .2s ease, background .2s ease;}
    .btn:hover{transform: translateY(-2px); background: var(--brand-2);}
    .btn.secondary{background:transparent;border:1px solid rgba(0,0,0,.25);color:var(--text);}
    .btn.secondary:hover{background:rgba(0,0,0,.08); border-color: rgba(0,0,0,.35); transform: translateY(-2px);}
    .hero-card{
...
void loop(){
  ...
  if(actionDone && millis()-lastActionTime > 1500){
    myServo.write(0);
